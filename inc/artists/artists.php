...
class ArteForaDoMuseu_Artists {
	...
	function set_slug() {
		$this->slug = apply_filters('artists_slug', 'country');
	}
	...
	function register_post_type() {
		...
		$labels = array( 
			'name' => __('Countries', 'arteforadomuseu'),
			'singular_name' => __('Country', 'arteforadomuseu'),
			'add_new' => __('Add country', 'arteforadomuseu'),
			'add_new_item' => __('Add new country', 'arteforadomuseu'),
			'edit_item' => __('Edit country', 'arteforadomuseu'),
			'new_item' => __('New country', 'arteforadomuseu'),
			'view_item' => __('View country', 'arteforadomuseu'),
			'search_items' => __('Search countries', 'arteforadomuseu'),
			'not_found' => __('No country found', 'arteforadomuseu'),
			'not_found_in_trash' => __('No country found in the trash', 'arteforadomuseu'),
			'menu_name' => __('Countries', 'arteforadomuseu')
		);

		$args = array( 
			'labels' => $labels,
			'hierarchical' => false,
			'description' => __('Countries', 'arteforadomuseu'),
	...
	function admin_css() {
		wp_enqueue_style('artists-admin', $this->directory_uri . '/css/admin.css');
	}
	...
	function ajax_delete() {
		...
		$this->ajax_response(array('success_msg' => __('Country has been deleted', 'arteforadomuseu')));
	}
...
//				}
			...
			<div class="lightbox_content">
				<?php if($artists) : ?>
					<form id="add_to_existing_artist" class="clearfix">
						<input type="hidden" class="artwork_id_" name="artwork_id" value="<?php echo get_the_ID(); ?>" />
						<input type="hidden" class="artwork_id" name="artwork_id" value="<?php echo get_the_ID(); ?>" />
	...
	function add_artwork_button($artwork_id = false) {
		...
		if(!$this->can_edit_any())
			return;
		?>
		<a class="add_artwork_to_artist" data-artwork="<?php echo $artwork_id; ?>" data-artwork-title="<?php echo get_the_title($artwork_id); ?>" href="#"><span class="lsf">&#xE041;</span> <?php _e('Register country', 'arteforadomuseu'); ?></a>
		<?php
	}
	...
	function hook_remove_artwork_button() {
		if($this->is_singular) {
			global $post;
			$this->remove_artwork_button($this->country->ID, $post->ID);
		}
	}
	...
	function delete_button($post_id = false) {
		...
		<form id="delete_artist">
			<input type="hidden" name="post_id" value="<?php echo $post_id; ?>" />
			<button class="remove" title="<?php _e('Permanently delete this country', 'arteforadomuseu'); ?>"><span class="lsf">&#xE10f;</span> <?php _e('Delete', 'arteforadomuseu'); ?></button>
		</form>
		<?php
	}
